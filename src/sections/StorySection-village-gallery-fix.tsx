...
function StoryCarousel({ data, startIndex = 0 }: { data: StorySectionData; startIndex?: number }) {
...
function StoryMedia({ data }: { data: StorySectionData }) {
  ...
  const hasSupportingGallery = data.gallery?.length && !reduceMotion;

  if ((data.id === "story" || data.id === "village") && data.videoId && hasSupportingGallery) {
    const videoAlt = data.id === "village" ? "วิดีโอมุมมองหมู่บ้านกาบกระบือ" : "วิดีโอเรื่องราวของกาบกระบือ";
    const variant = data.id === "village" ? "village" : "story";
    const supportingStartIndex = data.id === "village" ? 1 : 0;

    return (
      <div className="grid items-center gap-4 sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] sm:gap-5">
        <LazyStoryVideo src={`${basePath}/videos/${data.videoId}`} alt={videoAlt} fallbackSrc={fallbackSrc} variant={variant} />
        <div className="overflow-hidden rounded-[2rem] border border-ink/10 bg-rice/70 shadow-[0_30px_80px_-35px_rgba(31,33,27,0.35)]">
          <StoryCarousel data={data} startIndex={supportingStartIndex} />
        </div>
      </div>
    );
  }
